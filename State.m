classdef State < matlab.mixin.SetGet & SystemInfo
    % State class provides a 1-dim. wavefunction with position $q$ and momentum $p$ basis (representation).
    ...
    methods
        function obj = State(sysinfo, basis, vec, eigenvalue)
        ...
        function obj = tostate(obj, vec)
            ...
            obj = State(obj.sysinfo, obj.basis, vec);
        end
        
        function obj = double(obj)
            % truncate to double 
            sysinfo = SystemInfo(obj.dim, double(obj.domain) );
            obj = State(sysinfo, obj.basis, double(obj.y), double(obj.eigenvalue));            
        end
        ...
        function obj = q2p(obj)
            ...
            obj = State(obj.sysinfo, 'p', vec/sqrt(obj.dim), obj.eigenvalue);
        end
        ...
        function obj = p2q(obj)
            ...
            %vec = ifft(obj.y);                
            obj = State(obj.sysinfo, 'q', vec*sqrt(obj.dim), obj.eigenvalue);
        end
        
        function obj = qrep(obj)
            % return $y = <q|ψ>$ State class
        ...
        function [X, Y, mat] = hsmrep(varargin)
            % return $ρ(qc,pc) = |<cs_{(qc,pc)}|ψ>|^2$ where cs is cocherent state.
            % options:
            %    gridnum: positive integer 
            %        number of grid, 
            %        default 50
            %    vrange: 2x2 array such as [xmin xmax; ymin ymax]
            %        domain of hsmrep
            %    is
            %
            par = inputParser;
        ...
        function obj = plus(v1, v2)
            ...
            if strcmp(class(v2), 'State')
                v2 = v2.y;
            end
            v3 = v1.y + v2;
            obj = State(v1.sysinfo, v1.basis, v3);
        end
        ...
        function obj = times(v1, a)
            ...
            obj = State(v1.sysinfo, v1.basis, v1.y .* a);
        end
        ...
        function obj = normalize(obj)
            v = obj.y;
            a = dot(v, v);
            obj = State(obj.sysinfo, obj.basis, v/sqrt(a));
        end
        ...
        function obj = coherent(obj, qc, pc, varargin)
            ...
            if periodic
                ...
                a = dot(v, v);
                obj = State(obj.sysinfo, 'q', v / sqrt(a));
                if strcmp(basis, 'p')
                    obj = obj.q2p();
                end
            elseif ~periodic
        ...
        function obj = delta(obj, xc, varargin)
            ...
            obj = State(obj.sysinfo, obj.basis, y);            
        end               
